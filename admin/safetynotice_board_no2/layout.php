...
<title>レイアウト編集 - 安全掲示板 No.2</title>
...
const TYPE_ICONS  = { media:'🖼', text:'📝', accident:'🏆', notice:'📢', disaster:'🚨' };
...
function renderBoard() {
  ...
  panels.forEach((p, idx) => board.appendChild(createPanelEl(p, idx)));
}
...
function panelBodyHtml(p) {
  ...
  switch (p.type) {
    ...
    case 'text':
      return `<div style="position:absolute;inset:0;padding:8px;color:rgba(255,255,255,.85);font-size:14px;white-space:pre-wrap;word-break:break-all;overflow:hidden;">${escHtml(c.text||'') || '<span style="opacity:.4">テキストなし</span>'}</div>`;
    case 'accident': {
    ...
    case 'disaster': {
      ...
      return `<div class="placeholder">🚨 ${cnt ? cnt + ' 件' : '速報なし'}</div>`;
    }
    default: return '';
  }
}
...
function openViewBoard() {
  window.open('/view_board/safetynotice_board_no2/index.php', '_blank');
}
